...
def hamming_distance(X, X_train):
    ...
    return spatial.distance.cdist(X,X_train,'hamming') * X.shape[1]
...
def p_y_x_knn(y, k):
    ...
    for yi in y:
        for i in range (1,5):
           result = np.count_nonzero(np.array(yi[:k])==i)/k
...
def classification_error(p_y_x, y_true):
    ...
    """
    pyx = np.fliplr(p_y_x)
    yt2 = pyx.shape[1] - np.argmax(pyx, axis=1)

    return np.count_nonzero(np.subtract(yt2,y_true)) / y_true.shape[0]
...
def model_selection_knn(Xval, Xtrain, yval, ytrain, k_values):
    ...
    sortedl = sort_train_labels_knn(hamming_distance(Xval,Xtrain), ytrain)
    errors = []

    for i in range(len(k_values)):
        errors.append(classification_error(p_y_x_knn(sortedl,k_values[i]),yval))
